			...
			this.options.messages.forEach( (message, i) => {
				setTimeout( () => {
					this.$chat.insertAdjacentHTML(this.options.adding ?? 'beforeEnd', 
						`<div class="fakechat fakechat${this.options.theme ? this.options.theme.theme ?? 'default' : 'default'} fakechat_${this.options.theme ? this.options.theme.angle ?? 'rounded' : 'rounded'} ${this.options.animation ? this.options.animation.class ?? '' : ''} ${this.options.animation ? this.options.animation.function ?? '' : ''}"><span>${message.text}</span></div>`
						);
				}, 1000 + message.timer);
			});
		}
